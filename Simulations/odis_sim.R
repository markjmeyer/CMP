...
#   modified:   04/06/23
#
#   Overdispersed CMP, lambda = 3, nu = 0.5
...
##### MSE #####
mseLam  <- matrix(apply((lamMat - lambda)^2, c(2,3), median), nrow = 6, byrow = FALSE,
                  dimnames = list(Models = modNames, `Sample Sizes` = n))
mseNu   <- matrix(apply((nuMat - nu)^2, c(2,3), median), nrow = 6, byrow = FALSE,
                  dimnames = list(Models = modNames, `Sample Sizes` = n))
...
##### coverage #####
coverLam  <- matrix(apply(cov_lam, c(2,3), mean), nrow = 6, byrow = FALSE,
                    dimnames = list(Models = modNames, `Sample Sizes` = n))
coverNu   <- matrix(apply(cov_nu, c(2,3), mean), nrow = 6, byrow = FALSE,
                    dimnames = list(Models = modNames, `Sample Sizes` = n))
...
xtable(mseLam, digits = 4)
xtable(mseNu, digits = 4)

xtable(coverLam, digits = 2)
xtable(coverNu, digits = 2)
